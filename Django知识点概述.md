...
4. `JsonResponse`（`HttpResponse`的子类型）对象
    ...
    >>> response = JsonResponse({'foo': 'bar'})#返回json数据
    >>> response.content
    ...
    >>> response['cotent-type'] = 'application/pdf';#返回文件
    >>> response['content-disposition'] = 'inline#打开; filename="xyz.pdf"'
    >>> response['content-disposition'] = 'attachment#下载; filename="xyz.pdf"'
